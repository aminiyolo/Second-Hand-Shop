...
> 1. ### 랜딩페이지의 검색 기능과 카테고리 필터링 기능을 구현하는 부분에서 많은 어려움이 있었다. 이전 진행했던 레시피 프로젝트는 API 요청을 할때 쿼리값만 변경해주면 알아서 필터링된 데이터를 받을 수 있었다. 하지만, 이번 프로젝트에서는 스스로 서버 부분에서 프론트에서 받아온 데이터를 가공해서 필터링한 결과를 보여줘야 하다보니 그러한 점에서 어려움이 있었다. -> 이번 경험을 통해 클라이언트 사이드와 서버사이드의 데이터 통신 흐름의 구조를 이해할 수 있게 되었다.
>    처음 상태관리를 Redux 사용법을 잘 모르기도 하고 사용하기가 두려워 SWR를 이용하여 진행 하였었습니다. 하지만, 상태관리의 불편함을 느껴 Redux와 Redux toolkit을 적용하기로 결심한 결과, 리덕스를 적용하는 과정에서 많은 어려움을 겪었습니다(actionCreate, store 등등). 하지만, 이 과정을 통해 Redux 사용에 대한 두려움을 극복하고, 자신감을 얻을 수 있었습니다.

> 2. ### 이전까지는 axios에 대해 자세히 모르고 단순 기능만을 사용했었지만, 이번에 axios의 인스턴스 활용법, axios 요청 config 설정(header 설정 등)과 여러 기능에 대해 공부하고 사용하는 법에 알 수 있는 좋은 기회가 되었다.

> 3. ### 실시간 채팅을 구현하기 위햇 socket을 사용해야 했었는데, 한번도 공부해본적 없고, 사용해본적도 없었기 때문에 이번 프로젝트에서 가장 큰 어려움으로 다가왔다.
> 4. ## 이번 프로젝트를 통해 React, Javascript, CSS, HTML등 웹 전반적인 흐름에 대해 경험을 쌓을 수 있었다. 그리고, antd 보단 material-ui가 npm 트렌드에서 앞서는 것을 보고 다음 프로젝트에는 antd 대신 material-ui를 한번 사용해보고 싶다는 생각이 들었다.
